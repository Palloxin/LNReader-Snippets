...
.replace(/(“\w+)\.”\./g, '$1”.')
.replace(/([\"“”])(?<!\=\")(?!>|\s[\"“”])([^\"“”<]+)([\"”])(?<!=\")/g, '→→$1$2$3←←')
.replace(/\,([\"”]←←(?!<?(?:[^\"“”<]+(?:[\"”“](?=[a-z])|<(?!\/p>)))+[a-z](?<=[\"”“].))(?:<\/(?:strong|em|span|[bai])>)*|[\'\]’](?=\W))/g, '$1,')//regex101.com/r/ni3BdT/14
.replace(/←←|→→/g, '')
.replace(/,(?:,[, ]*|(?![\s\d\”\"\’;])(?<=\D\,))/g, ', ')
//↑↑↑↑
//↓↓↓  — apostrophe ( ' => ’ )
...
.replace(/(♠+)><(♠+)>(?<!\1>)(.+?)<\/\2><\/\1>/g, '♠♠>$3</♠♠>')
.replace(/(?:<(♠+)>(?:[^<]*<(?!\/?p>))+\/\1>(?:[^<]*<(?!\/?p>)[^>]+>)*){2,}/g, _ => `${_.replace(/<\/♠+>(.*?)<♠+>/g, '$1')}`)//clog
.replace(/(<\/♠+>\s*(?:<[^>]+>)*)([\!\?]+|[\;\.\:\,])/g, '$2$1')
.replace(/(♠+(?<=\/♠+)>(?:<[^>]+>)*)\s+(?!<\/p>)/g, '$1  ')//n+h space > normal space
.replace(/([”\"]\.?(?=<)|<♠+>)([“\"]|<\/♠+>)/g, '$2$1')
.replace(/([“\"])(<♠+>)([^♠\/]+)(<\/♠+>)([”\"]\s*)/g, (_, a1,a2,a3,a4,a5) => `${a2}${a1}${a3}${a5.replace(/\s+/, '')}${a4}`)
