...
//↓
.replace(/<span>(?:[^](?![/<]span>|<\/?p>))+?<\/span>(?:(?:<[^>]+>|\s)*?<span>(?:[^](?![/<]span>|<\/?p>))+?<\/span>)+/g, _ => `${_.replace(/<\/(span|strong)>([^]*?)<\1>/g, '$2')}`)//tag clog - jsbench.me/6emevgz4ox - regex101.com/r/kBlxED
//↑
.replace(/<br>\s*(?=<\/?p>)/g, '')
///↑↑↑↑↑
//↓↓↓ —
...
.replace(/(♠+)><(♠+)>(?<!\1>)([^]+?)<\/\2><\/\1>/g, '♠♠>$3</♠♠>')
.replace(/<(♠+)>(?:[^](?![/<]\1>|<\/?p>))+?<\/\1>(?:(?:<[^>]+>|\s)*<\1>(?:[^](?![/<]\1>|<\/?p>))+?<\/\1>)+/g, _ => `${_.replace(/<\/♠+>([^]*?)<♠+>/g, '$1')}`)//clog
.replace(/(<\/♠+>\s*(?:<[^>]+>)*)([\!\?]+|[\;\.\:\,])/g, '$2$1')
