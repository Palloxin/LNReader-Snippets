...
.replace(/\b((?:[1-9]|1[012]|0\d(?=:\d))(?<!(?::|\d-)\d\d?)(?::[0-5]\d)?(?:-| ?[AaPp]\.?[mM](?=\.[^\.]|\b)))/g, '$1••••')//♦
...
.replace(/\b(?:[1-9]|1[01]|[A-Za-z]+) o’clock(\w+)( (?:at|in the)(?<=\D(?:[7-9]|1[01]) (?:at|in the)) (?:night|evening)| in(?<=[1-9] in) the afternoon)?\b/g, (_, a,b) => `${b?+a+12:a} o’clock${b||''}`)
//↑
.replace(/(:\d\d|\b[AP]M)\.(?=[\.\,])/g, '$1')
.replace(/°°-°°/g, ' - ')//sixth spaces
.replace(/••••(?=\d)/g, '')
;
//fiddle: https://jsfiddle.net/a2z3omye/
/////↑↑↑↑↑↑↑↑↑↑ – END
