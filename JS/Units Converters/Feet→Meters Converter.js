...
.replace(/⋮⋮⋮feet\b(?<=\b(?<!\-)(?:(?:twen(?=ty)|thir|fou?r|fif|six|seven|eigh|nine)(?:teen|ty)(?:(?<=y)\-(?:one|two|three|four|five|six|seven|eight|nine))?|(?:two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)|\d+(?:\.\d+)?|(?:a|one)(?<=[^a-z\-][a-z]+)(?= (?:hundred|thousand) ⋮))\b( and a half| (hundred|thousand))? ⋮⋮⋮feet)/g, '⋮⋮⋮ↂfeet')
