...
.replace(/™™™f(%+t²?[ \-])(?=-?(?:tall|thick|long|wide|height|lower|radius|distance|away|from|in (?:length|height|diameter)|(?:deep|high)(?! in(?:to)?\b)|of distance|farther down)\b)/g, '⋮⋮⋮f$1√√')
...
.replace(/÷×(\([\d\.]+m²?\))([ \-])√√((?:in |of (?=distance))?[a-z]+)/g, '$2$3 $1')
.replace(/÷×/g, '')
.replace(/(?:⋮⋮⋮|™™™)f(%+)t(²)?(([ \-])√√)?/g, (_,z, a,b,c) => `${a?'square ':''}f${z==='%'?'oo':'ee'}t${b?c:''}`)
